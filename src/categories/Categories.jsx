  ...
  return (
    ...
    <div className='main-body flex-wrap' style = {{height: '100vh'}}>
      ...
      {categories.length === 0 ? 'Loading...' : categories.map((category, index) => (
        <p key={index} className="card m-3" style={{width: "20rem"}}>
          <Link to={`/categories/${category.name}`} className='d-flex justify-content-center text-dark fw-bold'  >{category.name}</Link>
        </p>
      ))}
